...
function bearingBetween(playerX, playerY, islandX, islandY) {
  const angle = Math.atan2(islandX - playerX, playerY - islandY) * (180 / Math.PI);
...
function clockState(island, now) {
  if (now < island.disappearsAt) return { value: countdown(island.disappearsAt, now), label: "until disappearance" };
  if (now < island.respawnsAt) return { value: countdown(island.respawnsAt, now), label: "until respawn" };
...
function validate(data) {
  ...
  if (Number.isNaN(new Date(data.occupiedAt).getTime())) return "Enter a valid occupation time.";
  if (data.activeDuration <= 0) return "Active duration must be greater than zero.";
  if (data.activeDuration > limit.hours * 3600) return `${limit.label} cannot stay occupied longer than ${limit.hours} hours.`;
  if ([data.playerX, data.playerY, data.islandX, data.islandY].some(Number.isNaN)) return "Player and island coordinates are required.";
  return "";
}
function makeIsland(data) {
  const occupiedAt = new Date(data.occupiedAt);
  const disappearsAt = new Date(occupiedAt.getTime() + data.activeDuration * 1000);
  const respawnsAt = new Date(disappearsAt.getTime() + limits[data.type].respawnDays * 86400000);
  return { ...data, id: crypto.randomUUID(), disappearsAt, respawnsAt, distance: distanceBetween(data.playerX, data.playerY, data.islandX, data.islandY), bearing: bearingBetween(data.playerX, data.playerY, data.islandX, data.islandY) };
}
function render() {
  const now = new Date();
  ...
  list.innerHTML = islands.map((island) => {
...
function refreshTypeLimit() {
  $("#duration-limit").textContent = `Maximum ${String(limits[typeInput.value].hours).padStart(2, "0")}:00:00 for ${limits[typeInput.value].label}`;
}
...
list.addEventListener("click", (event) => {
  ...
  if (button.dataset.action === "remove") islands = islands.filter((island) => island.id !== card.dataset.id);
  render();
});
clearAll.addEventListener("click", () => { islands = []; render(); });
setDefaultDate();
refreshTypeLimit();
render();
